...
- 跨 session 唯一入口是 `.agent/HANDOFF.md`，不超过 4 KiB/五个 `must_read`；语义完整指目标、决定、事实、证据、进展、下一步、阻塞与风险可恢复，不保存聊天、思维链或源码副本。
- Summer native 模式以 `.agent/ledger/` 为权威状态，Handoff 由当前 Canonical 状态派生；GSD 模式以 `.planning/` 为权威状态，Handoff 只保存指针和恢复命令；Direct 模式只保存简短快照。
...
- 优先用 `summer resume` / `summer doctor` 读取和诊断；Native v2 可重建缺失 Handoff，但漂移、生命周期冲突或不安全引用必须 fail-closed，不能从聊天猜测或静默覆盖。
