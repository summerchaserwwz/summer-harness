...
Matt Skills 很适合做需求拷问、bug 诊断、TDD、代码设计和领域建模等能力模块，但它们不提供稳定的任务主账本、完成门禁或唯一跨 session 状态。上游 `ask-matt` 会建立独立的 idea-to-ship 流程，因此不作为 Summer 的默认入口。

因此 Summer 只按需路由到少数能力：逐问需求用 `grilling`，其余使用 `diagnosing-bugs`、`codebase-design`、`domain-modeling`、`tdd`。生命周期仍由 Direct、Summer 或 GSD 三者之一独占。
